...
int main(int argc, char *argv[])
{
   ...
   for (int pow_of_two = 10; pow_of_two <= 27; pow_of_two++) {
      ...
      test_sum = do_sum(energy, ncells);
      cpu_time = cpu_timer_stop(cpu_timer);
      printf("Standard double sum: result=%-17.16lg diff=%10.4lg rel_diff=%10.4lg runtime=%lf\n",
      ...
      cpu_time = cpu_timer_stop(cpu_timer);
      printf("Pairwise sum:        result=%-17.16lg diff=%10.4lg rel_diff=%10.4lg runtime=%lf\n",
            test_sum, test_sum - accurate_sum, (test_sum - accurate_sum) / accurate_sum, cpu_time);
      ...
      cpu_time = cpu_timer_stop(cpu_timer);
      printf("Kahan sum:           result=%-17.16lg diff=%10.4lg rel_diff=%10.4lg runtime=%lf\n",
            test_sum, test_sum - accurate_sum, (test_sum - accurate_sum) / accurate_sum, cpu_time);
      ...
      cpu_time = cpu_timer_stop(cpu_timer);
      printf("Knuth sum:           result=%-17.16lg diff=%10.4lg rel_diff=%10.4lg runtime=%lf\n",
            test_sum, test_sum - accurate_sum, (test_sum - accurate_sum) / accurate_sum, cpu_time);


      cpu_timer_start(&cpu_timer);
      long double test_sum_ld = do_longdouble_sum(energy, ncells);
      cpu_time = cpu_timer_stop(cpu_timer);
      printf("Long double sum:     result=%-17.16Lg diff=%10.4Lg rel_diff=%10.4Lg runtime=%lf\n",
            test_sum_ld, test_sum_ld - (long double)accurate_sum,
            (test_sum_ld - (long double)accurate_sum) / (long double)accurate_sum, cpu_time);
